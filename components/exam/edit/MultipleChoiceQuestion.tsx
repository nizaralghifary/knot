...
export function MultipleChoiceQuestion({ question, onUpdate }: MultipleChoiceQuestionProps) {
  ...
  const handleRadioChange = (selectedIndex: string) => {
    ...
    if (selectedOptionText.trim()) {
      onUpdate({ correct_answer: selectedOptionText });
    }
  };
  ...
  const handleOptionInputChange = (optIndex: number, value: string) => {
    ...
    const updates: Partial<MCQType> = { options: newOptions };
    
    if (question.correct_answer === options[optIndex]) {
      updates.correct_answer = value;
    }
          ...
          <div key={optIndex} className="space-y-2">
            <div className="flex items-center gap-3">
              <RadioGroupItem 
                ...
                disabled={!option.trim()} // Disable jika option kosong
              />
              <Label 
